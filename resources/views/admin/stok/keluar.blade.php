<x-admin-layout title="Produk Keluar">
    <div class="mb-8">
        <h1 class="text-3xl font-black text-gray-900 uppercase tracking-tighter">Produk Keluar</h1>
        <p class="text-[13px] text-gray-500 font-medium mt-1">Kurangi stok produk (koreksi, retur ke supplier, dll).</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="bg-white rounded-[2.5rem] p-8 shadow-sm border border-gray-100">
            <h3 class="text-sm font-black text-gray-900 uppercase tracking-widest mb-6">Form Produk Keluar</h3>
            ...
            <form action="{{ route('admin.stok.keluar.store') }}" method="POST" class="space-y-5">
                @csrf
                ...
                <div>
                    <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Jumlah Keluar <span class="text-red-500">*</span></label>
                    <input type="number" name="jumlah" value="{{ old('jumlah', 1) }}" min="1" required
                        class="w-full bg-gray-50 border border-gray-200 rounded-2xl px-4 py-3 text-sm font-bold focus:ring-2 focus:ring-red-400 focus:border-transparent"
                        placeholder="Masukkan jumlah produk yang keluar">
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="keterangan" value="{{ old('keterangan') }}"
                        class="w-full bg-gray-50 border border-gray-200 rounded-2xl px-4 py-3 text-sm font-bold focus:ring-2 focus:ring-yellow-400 focus:border-transparent"
                        placeholder="Contoh: Koreksi stok, Retur ke supplier, Produk rusak, dll.">
                </div>

                <button type="submit"
                    class="w-full py-4 bg-red-500 text-white text-[11px] font-black uppercase tracking-widest rounded-2xl hover:bg-red-600 transition shadow-sm flex items-center justify-center gap-2">
        ...
        <div class="bg-white rounded-[2.5rem] p-8 shadow-sm border border-gray-100">
            ...
            <div class="space-y-3 max-h-[480px] overflow-y-auto pr-1">
                ...
                <div class="flex items-center justify-between p-3 rounded-2xl border border-gray-100 hover:bg-gray-50 transition">
                    ...
                    <span class="text-[13px] font-black {{ $p->stok > 5 ? 'text-green-600' : 'text-red-500' }}">
                        {{ $p->stok }} pcs
                    </span>
                </div>
                @empty
                <p class="text-sm text-gray-400 text-center py-8">Tidak ada produk dengan stok tersedia.</p>
                @endforelse
            </div>
        </div>
    </div>

                ...
                <nav class="flex-1 space-y-1">
                    ...
                    <details class="group mb-1" {{ request()->routeIs('admin.produk.*') ? 'open' : '' }}>
                        <summary class="flex items-center justify-between w-full rounded-xl transition cursor-pointer list-none {{ request()->routeIs('admin.produk.*') ? 'bg-gray-100' : 'hover:bg-gray-50' }}" style="list-style: none;">
                            <a href="{{ route('admin.produk.index') }}" class="flex-1 flex items-center gap-4 px-4 py-3.5 text-lg font-black {{ request()->routeIs('admin.produk.*') ? 'text-gray-900' : 'text-gray-500 hover:text-gray-900' }}">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 {{ request()->routeIs('admin.produk.*') ? 'text-gray-900' : 'text-gray-400 group-hover:text-gray-900' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <div class="pl-4 pr-2 pt-2 space-y-1">
                            <a href="{{ route('admin.produk.barang-masuk') }}" class="flex items-center gap-3 w-full px-4 py-2.5 text-sm font-bold rounded-xl transition {{ request()->routeIs('admin.produk.barang-masuk') ? 'text-blue-600 bg-blue-50' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-50' }}">
                                <div class="w-1.5 h-1.5 rounded-full {{ request()->routeIs('admin.produk.barang-masuk') ? 'bg-blue-600' : 'bg-gray-300' }}"></div>
                                Produk Masuk
                            </a>
                            <a href="{{ route('admin.produk.barang-keluar') }}" class="flex items-center gap-3 w-full px-4 py-2.5 text-sm font-bold rounded-xl transition {{ request()->routeIs('admin.produk.barang-keluar') ? 'text-blue-600 bg-blue-50' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-50' }}">
